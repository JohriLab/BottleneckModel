...
source("analysis/equations/coalescence/WF_equations.R")
...
simulated_data <- simulated_data %>% filter(k == k)
    ...
    piT_plot <- ggplot(data, aes(x = m, y = piT, color = "#E69F00")) +
        ...
        geom_point(data = simulated_data, aes(x = m, y = piT), color = "#0072B2", size = 2) +
        geom_errorbar(data = simulated_data, aes(x = m, ymin = piT - piT_se, ymax = piT + piT_se), color = "#0072B2", width = 0.1, linewidth = 0.5, inherit.aes = FALSE) +
        facet_wrap(~k, scales = "free_y", nrow = 1) +
        scale_x_log10() +
        scale_y_continuous(labels = scientific) +
...
scale <- 1.25
ggsave("analysis/pi_plot.png", combined_plot, width = 10*scale, height = 8*scale, dpi = 300)

# # Save the plot
# ggsave("analysis/single_coalescent_plot.png", combined_plot, width = 7, height = 12, dpi = 300)
